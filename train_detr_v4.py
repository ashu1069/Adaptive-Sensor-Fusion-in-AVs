...
class DETR(nn.Module):
    def __init__(self, num_classes=91, num_queries=100, hidden_dim=256, 
        ...
        self.fusion_layer = nn.Conv2d(hidden_dim+1024, hidden_dim, kernel_size=1).to(self.device)
...
def validate(model, val_loader, criterion, device, epoch):
    ...
    if all_pred_labels and all_true_labels:  # Check if we have any valid predictions
        ...
        plt.savefig(f'results/detr/fused/confusion_matrix_epoch_{epoch + 1}.png')
        plt.close()
...
# Add after the model initialization and before training loop
...
# # Initialize criterion with the computed weights
# criterion = DETRLoss(num_classes=8, class_weights=class_weights).to(device)

# # Training loop modifications
# optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
# scheduler = CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=1e-6)

# # Add after model, optimizer, and scheduler initialization but before training loop
# # best_model_path = 'best_model.pth'  # Adjust path as needed
# best_model_path = None # Adjust path as needed
...
# # Modify the training loop to start from loaded epoch
# mlflow.end_run()
# with mlflow.start_run() as run:
    ...
    for epoch in range(start_epoch, num_epochs):  # Changed from range(num_epochs)
        if epoch == start_epoch:  # Changed from epoch == 0
        ...
        print(f"Epoch {epoch + 1}, Training Loss: {avg_train_loss}, cl_loss: {avg_cl_loss}, bo_loss: {avg_bo_loss}, giou_loss: {avg_giou_loss}")
...
def inference_mode(model_path, val_loader, device, iou_threshold=0.5):
    # Load model
    model = DETR(num_classes=8, num_queries=12, device=device, hidden_dim=128)
    ...
    plt.savefig(f'results/detr/fused/confusion_matrix_inference_iou_{iou_threshold}.png')
    plt.close()
...
def train_detr(data_dir, num_epochs=300, batch_size=32, learning_rate=1e-4, num_queries=12, patience=15, 
               best_model_path=None, wanted_labels=[0,4,5,6,7], num_hidden_dim=256):
